...
// Batch cache pages using Playwright server
async function cacheWithPlaywrightBatch(urls, maxDepth = 0, concurrency = 3) {
  console.log(
    `🎭 Attempting to batch cache ${urls.length} URLs with Playwright`
  );

  try {
    const response = await fetch(
      `${CONFIG.PLAYWRIGHT_SERVER_URL}/api/cache/batch`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        signal: AbortSignal.timeout(
          CONFIG.PLAYWRIGHT_TIMEOUT * urls.length || 300000
        ),
      }
    );
    ...
    // Process successful results and store in Chrome storage
    const successfulResults = [];

    for (const result of batchResult.results) {
      try {
        // Get the cached content for each successful URL
        const contentResponse = await fetch(
          `${CONFIG.PLAYWRIGHT_SERVER_URL}/api/content/${result.cacheHash}`
        );
        ...
        console.error(
          `Error processing cached content for ${result.url}:`,
          error
        );
      }
    }
    ...
    console.error("❌ Playwright batch cache error:", error);
    throw error;
  }
}
...
async function runSmartCaching(token, zone, queries, resultsPerQuery = 5) {
  try {
    ...
    if (usePlaywright && toScrape.length > 0) {
      try {
        // Use batch caching for multiple URLs
        console.log(`🚀 Using batch caching for ${toScrape.length} URLs`);
        const batchResult = await cacheWithPlaywrightBatch(
          toScrape,
          0,
          resultsPerQuery
        );
        ...
        // For failed URLs, try simple scraping as fallback
        const failedUrls = batchResult.errors.map((e) => e.url);
        if (failedUrls.length > 0) {
          console.log(
            `🔄 Attempting HTML scraping fallback for ${failedUrls.length} failed URLs`
          );
        ...
        console.log(
          `📊 Batch caching results: ${batchResult.successful} playwright, ${
            results.filter(
              (r) => r.status === "cached" && r.method === "simple"
            ).length
          } simple, ${
        ...
        console.error(
          "Batch caching failed, falling back to sequential:",
          batchError
        );

        // Fallback to sequential caching
        for (const url of toScrape) {
          try {
            ...
            // Try simple scraping as fallback
            try {
              const { content, title } = await scrapePage(url);
              if (!content) throw new Error("Empty content");
              const pageData = {
              ...
              console.warn("Both caching methods failed for", url, fallbackErr);
              results.push({
                url,
                status: "error",
                error: String(fallbackErr?.message || fallbackErr),
      ...
      // No Playwright - use simple scraping with parallel processing
      console.log(`📄 Using simple scraping for ${toScrape.length} URLs`);
      ...
      for (let i = 0; i < toScrape.length; i += concurrency) {
        ...
        const batchPromises = batch.map(async (url) => {
          try {
            ...
            await saveCachedPage(pageData);
            return { url, status: "cached", method: "simple" };
          } catch (err) {
