document.addEventListener("DOMContentLoaded", function () {
  ...
  async function viewCachedPage(url) {
    try {
      ...
      if (page) {
        // Check if page was cached with Playwright (has cacheHash)
        if (page.method === "playwright" && page.cacheHash) {
          // Open via server URL to preserve asset references
          const config = await getConfig();
          const serverUrl =
            config?.PLAYWRIGHT_SERVER_URL || "http://localhost:3000";
          const cachedUrl = `${serverUrl}/cached/${page.cacheHash}/`;
